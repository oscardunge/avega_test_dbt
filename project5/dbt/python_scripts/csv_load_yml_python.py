...
def load_to_staging(client, schemas_config, csv_dir):
    """Loads CSV data into BigQuery staging tables with error handling."""
    files_loaded = 0
    ...
    error_logs = []
    print(f" {schemas_config}")

    for table_name in schemas_config.get("yml_table_definitions", {}):
        csv_file_path = os.path.join(csv_dir, f"{table_name}.csv")

        # yml_ddl = schemas_config.get("yml_ddls", {}).get(table_name)  # Use yml_ddls
        yml_individual_table_def = schemas_config.get("yml_table_definitions", {}).get(table_name)
        if yml_individual_table_def:
            print(f"Table Name: {table_name}")
            print(f"YML individual_table_def: {yml_individual_table_def}")

        table_ref = create_staging_table_if_not_exists(client, project_id, dataset_id, table_name, yml_individual_table_def, csv_file_path)
        ...
        try:
            ...
            schema = []
            if yml_individual_table_def:  # Use yml_individual_table_def
                for field in yml_individual_table_def:  # Use yml_individual_table_def
                    if field["name"] in df.columns:
                        if field["type"] == "DATE":
                            try:
                                ...
                                try:
                                    df[field["name"]] = pd.to_datetime(df[field["name"]], format="%Y-%m-%d %H:%M:%S", errors='coerce').dt.strftime('%Y-%m-%d')
                                except (pd.errors.ParserError, ValueError):
                        ...
                        logging.warning(f"Column '{field['name']}' from schema not found in CSV for {table_name}. Skipping this field.")
            else:
                pandas_schema = get_pandas_schema(csv_file_path)
                if pandas_schema:
                    for col_name, dtype in pandas_schema.items():
